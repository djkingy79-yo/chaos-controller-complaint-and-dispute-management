...
Deno.serve(async (req) => {
  try {
    ...
    if (isDeadlineAutomation) {
      ...
      try {
        const existing = await graphRequest(accessToken, `/me/todo/tasks?$filter=subject eq '[Chaos Controller] ${dl.title.replace(/'/g, "''")}'`);
        const match = (existing?.value || []).find(t => t.subject?.includes('[Chaos Controller]'));
        if (match) {
          // Update existing task
          await graphRequest(accessToken, `/me/todo/tasks/${match.id}`, {
            method: 'PATCH',
            body: JSON.stringify({
              subject: `[Chaos Controller] ${dl.title}`,
              body: { content: `Case: ${caseItem.title}\nOrganisation: ${caseItem.organisation_name || 'N/A'}\nType: ${dl.deadline_type || 'Deadline'}`, contentType: 'text' },
      ...
      await graphRequest(accessToken, '/me/todo/tasks', { method: 'POST', body: JSON.stringify(task) });
    ...
    // Get existing tasks
    const existingTasksRes = await graphRequest(accessToken, '/me/todo/tasks?$filter=status ne \'completed\'');
    const existingTasks = existingTasksRes?.value || [];
    const existingTaskMap = new Map();
    existingTasks.forEach(t => {
      if (t.subject?.includes('[Chaos Controller]')) {
        existingTaskMap.set(t.subject, t);
      }
    });
    ...
    for (const dl of deadlines) {
      ...
      const existingTask = existingTaskMap.get(taskSubject);
      ...
      if (existingTask) {
        await graphRequest(accessToken, `/me/todo/tasks/${existingTask.id}`, {
          method: 'PATCH',
          body: JSON.stringify(taskData)
        });
        updated++;
        existingTaskMap.delete(taskSubject);
      } else {
        await graphRequest(accessToken, '/me/todo/tasks', {
          method: 'POST',
          body: JSON.stringify(taskData)
        });
        created++;
    ...
    for (const [subject, task] of existingTaskMap) {
      try {
        await graphRequest(accessToken, `/me/todo/tasks/${task.id}`, { method: 'DELETE' });
      } catch (_) {}
    }
